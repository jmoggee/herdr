...
Set `tab_numbers` under `[ui]` to control when a tab's number appears as its own chip. The number is the tab's position, which is also its `prefix+<n>` switch key, so it renumbers automatically whenever tabs are closed or reordered.
...
- `"auto"` (default) shows a chip on named tabs. An unnamed tab already shows its number as its whole label, so it stays as it is.
- `"always"` shows a chip on every tab. Unnamed tabs become a compact chip rather than repeating the number.
- `"never"` shows no chip.

Tabs are automatically named until you rename them. Choose whether those names use application-provided terminal titles or concise tmux-style foreground commands:
...
`"terminal_title"` is the default. `"command"` produces labels such as `bash`, `nvim`, or `claude` and falls back to the launched shell until process detection completes. Process changes normally appear immediately; an executable that replaces another process in place can take a few seconds. An explicit rename stays fixed, and saving an empty rename restores automatic naming.

The chip takes its colors from the active theme. To match an existing status line — a tmux `window-status-format`, for example — override the tab row's elements individually:
...
`number_*` styles the chip and the unprefixed keys style the rest of the tab, each with an `active_*` counterpart for the focused tab. `bar_bg` is the row itself, behind and around the tabs — tmux's `status-style` background, which is often a step darker than the terminal's own. Any key you leave out keeps following the selected theme, so you can retint just the chip and leave the tab body alone. Values accept the same spellings as `theme.custom`.
