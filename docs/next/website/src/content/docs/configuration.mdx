...
Set `show_tab_numbers = "always"` if you want renamed tabs to keep their current switch number in the tab bar. Auto-named numbered tabs are not duplicated, so tab 2 still shows `2` instead of `2 2`.

Set `pane_border_style = "single"` for tmux-style internal split dividers without an outer pane frame. The focused pane keeps the same line weight and is shown with the accent color. With a simple two-pane split, one half of the shared divider is highlighted so the active side stays visible. The default `boxed` style keeps Herdr's existing full pane boxes and thick active terminal border.
