...
`number_*` styles the chip and the unprefixed keys style the rest of the tab, each with an `active_*` counterpart for the focused tab. Any key you leave out keeps following the selected theme, so you can retint just the chip and leave the tab body alone. Values accept the same spellings as `theme.custom`.
