...
fn render_settings_pane_borders(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    super::widgets::render_modal_description(
        frame,
        desc_area,
        "choose split pane border labels and tmux-style single borders",
        Style::default().fg(p.overlay1),
    );
    ...
    let rows = [
        (
            ...
            "show detected agent names in pane borders",
        ),
        (
            "agent labels off",
            !app.agent_border_labels_enabled(),
            "hide detected agent names unless panes are manually named",
        ),
        (
            "single borders",
            app.pane_border_style == crate::app::state::PaneBorderStyle::Single,
            "plain shared-style borders with focused pane in accent",
        ),
        (
            "boxed borders",
            app.pane_border_style == crate::app::state::PaneBorderStyle::Boxed,
            "classic full pane boxes with thick focused terminal border",
        ),
    ];

    for (idx, (label, enabled, description)) in rows.iter().enumerate() {
