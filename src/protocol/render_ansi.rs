...
fn color_to_sgr_underline(val: u32) -> String {
    ...
    match tag {
        0x00 => match val & 0xFF {
            0x00 => "59".to_owned(),
            0x01 => "58;5;0".to_owned(),
            0x02 => "58;5;1".to_owned(),
            0x03 => "58;5;2".to_owned(),
            0x04 => "58;5;3".to_owned(),
            0x05 => "58;5;4".to_owned(),
            0x06 => "58;5;5".to_owned(),
            0x07 => "58;5;6".to_owned(),
            0x08 => "58;5;7".to_owned(),
            0x09 => "58;5;8".to_owned(),
            0x0A => "58;5;9".to_owned(),
            0x0B => "58;5;10".to_owned(),
            0x0C => "58;5;11".to_owned(),
            0x0D => "58;5;12".to_owned(),
            0x0E => "58;5;13".to_owned(),
            0x0F => "58;5;14".to_owned(),
            0x10 => "58;5;15".to_owned(),
            _ => "59".to_owned(),
        },
...
fn underline_style_to_sgr_part(style: u8) -> Option<&'static str> {
    match style {
        1 => Some("4"),
        2 => Some("4:2"),
        3 => Some("4:3"),
...
fn build_sgr(fg: u32, bg: u32, underline_color: u32, modifier: u16, underline_style: u8) -> String {
    ...
    if let Some(underline) = underline_style_to_sgr_part(underline_style) {
...
mod tests {
    ...
    #[test]
    fn color_to_sgr_underline_rgb() {
        assert_eq!(color_to_sgr_underline(0x02_11_22_33), "58;2;17;34;51");
    }
    ...
    fn build_sgr_produces_valid_sequence() {
        let sgr = build_sgr(0x00_00_00_02, 0x00_00_00_01, 0, 1, 0); // fg=Red, bg=Black, bold
    ...
    #[test]
    fn build_sgr_preserves_curly_underline_shape() {
        let sgr = build_sgr(0x00_00_00_00, 0x00_00_00_00, 0, 8, 3);
        assert!(
            sgr.contains("4:3"),
            "SGR did not contain undercurl: {sgr:?}"
        );
    }

    #[test]
    fn build_sgr_preserves_underline_color() {
        let sgr = build_sgr(0x00_00_00_00, 0x00_00_00_00, 0x02_11_22_33, 8, 3);
        assert!(
            sgr.contains("58;2;17;34;51"),
            "SGR did not contain underline color: {sgr:?}"
        );
    }

    #[test]
    fn blit_frame_preserves_curly_underline_shape() {
        let mut cell = make_cell("U", 0, 0, 8);
        cell.underline_style = 3;
        let frame = make_frame(1, 1, vec![cell]);

        let mut output = Vec::new();
        blit_frame_to(&mut output, &frame, None);

        assert!(
            output.windows(b"4:3".len()).any(|window| window == b"4:3"),
            "output did not contain undercurl SGR: {output:?}"
        );
    }

    #[test]
    fn blit_frame_preserves_distinct_underline_color() {
        let mut cell = make_cell("U", 0, 0, 8);
        cell.underline_style = 3;
        cell.underline_color = 0x02_11_22_33;
        let frame = make_frame(1, 1, vec![cell]);
        ...
        assert!(
            output
                .windows(b"58;2;17;34;51".len())
                .any(|window| window == b"58;2;17;34;51"),
            "output did not contain underline-color SGR: {output:?}"
        );
    }
    ...
    fn build_sgr_resets_previous_modifiers_when_cell_is_plain() {
        assert_eq!(
            build_sgr(0x00_00_00_00, 0x00_00_00_00, 0, 0, 0),
            "\x1b[0;39;49;59m"
        );
    }
